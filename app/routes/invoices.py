...
def invoice_create():
    ...
    try:
        ...
        if status == 'paid':
            # ==========================================
            # MODIFICADO: Usar el mÃƒÂ©todo con soporte de seriales
            # ==========================================
            ...
            if not success:
                ...
                return redirect(url_for('invoices.invoice_new'))
            # ==========================================
            # FIN MODIFICACIÃƒâ€œN
            # ==========================================

        # Guardar configuraciÃƒÂ³n actualizada
...
def invoice_update(invoice_id):
    ...
    segÃƒÆ’Ã‚Âºn regulaciones de la DGII.
    """
    invoice = Invoice.query.get_or_404(invoice_id)
    ...
    try:
        ...
        if old_status != new_status:
            ...
            if old_status == 'paid' and new_status != 'paid':
                success, error_msg = InvoiceInventoryService.update_inventory_for_invoice(invoice, action='add', user_id=current_user.id)
                if not success:
                    db.session.rollback()
                    flash(f'Error al restaurar inventario: {error_msg}', 'error')
                    return redirect(url_for('invoices.invoice_edit', invoice_id=invoice.id))

            # Si no estaba pagada y ahora sÃƒÆ’Ã‚Â­, descontar inventario
            elif old_status != 'paid' and new_status == 'paid':
                success, error_msg = InvoiceInventoryService.update_inventory_for_invoice(invoice, action='subtract', user_id=current_user.id)
                if not success:
                    db.session.rollback()
...
def export_csv():
    ...
    writer.writerow([
        'NÃƒÆ’Ã‚Âºmero', 'NCF', 'Tipo NCF', 'Fecha', 'Cliente', 'RNC/CÃƒÆ’Ã‚Â©dula',
        'Subtotal', 'ITBIS', 'Total', 'Estado', 'MÃƒÆ’Ã‚Â©todo de Pago'
    ])
...
# ============================================
# RUTA: CONFIGURACIÃƒÆ’Ã¢â‚¬Å“N DE FACTURACIÃƒÆ’Ã¢â‚¬Å“N
# ============================================

@invoices_bp.route('/settings', methods=['GET'])
@login_required
@any_permission_required('invoices.settings.view', 'invoices.settings.manage')
def settings():
    """
    Mostrar configuraciÃƒÆ’Ã‚Â³n de facturaciÃƒÆ’Ã‚Â³n
...
# ============================================
# RUTA: ACTUALIZAR CONFIGURACIÃƒÆ’Ã¢â‚¬Å“N
# ============================================
...
def settings_update():
    ...
    try:
        ...
        # ConfiguraciÃ³n Visual y Bancaria (NUEVO)
        ...
        db.session.commit()
        flash('ConfiguraciÃƒÆ’Ã‚Â³n actualizada exitosamente', 'success')
    except Exception as e:
        db.session.rollback()
        flash(f'Error al actualizar configuraciÃƒÆ’Ã‚Â³n: {str(e)}', 'error')
...
def update_ncf_sequence(ncf_type):
    ...
    if ncf_type not in NCF_TYPES:
        return jsonify({'success': False, 'message': f'Tipo de NCF "{ncf_type}" no vÃƒÆ’Ã‚Â¡lido'}), 400
...
def api_suggest_ncf_type(customer_id):
    ...
    Returns:
        JSON con el tipo de NCF sugerido y razÃƒÆ’Ã‚Â³n
    """
    ...
    # Obtener informaciÃƒÆ’Ã‚Â³n de la secuencia actual
...
def api_get_ncf_types():
    ...
    # Agregar informaciÃƒÆ’Ã‚Â³n de secuencia a cada tipo
...
def api_validate_ncf():
    ...
    if not ncf:
        return jsonify({
            'success': True,
            'valid': False,
            'message': 'El NCF no puede estar vacÃƒÆ’Ã‚Â­o'
        })
    ...
    return jsonify({
        'success': True,
        'valid': is_valid,
        'message': error_msg if error_msg else 'NCF vÃƒÆ’Ã‚Â¡lido y disponible'
    })
...
def api_search_customers():
    ...
    # Incluir informaciÃƒÆ’Ã‚Â³n para sugerir tipo de NCF
    results = []
    for c in customers:
...
def upload_logo():
    ...
    # Validar que se enviÃƒÆ’Ã‚Â³ un archivo
    if 'logo' not in request.files:
        return jsonify({'success': False, 'message': 'No se enviÃƒÆ’Ã‚Â³ ningÃƒÆ’Ã‚Âºn archivo'}), 400

    file = request.files['logo']

    # Validar que tenga nombre
    if file.filename == '':
        return jsonify({'success': False, 'message': 'No se seleccionÃƒÆ’Ã‚Â³ archivo'}), 400
    ...
    # Validar tamaÃƒÆ’Ã‚Â±o (mÃƒÆ’Ã‚Â¡ximo 2MB)
    if len(file.read()) > 2 * 1024 * 1024:
        file.seek(0)
        return jsonify({
            'success': False,
            'message': 'Archivo muy grande. MÃƒÆ’Ã‚Â¡ximo 2MB'
        }), 400
    ...
    try:
        ...
        # Actualizar configuraciÃƒÆ’Ã‚Â³n
...
def remove_logo():
    ...
    try:
        if settings.logo_path:
            # Eliminar archivo fÃƒÆ’Ã‚Â­sico
            logo_dir = os.path.join(current_app.root_path, 'static', 'logos')
